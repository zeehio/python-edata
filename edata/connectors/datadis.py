...
class AsyncDatadisConnector:
    ...
    async def _update_recent_queries(self, query: str, data: dict | None = None) -> None:
        ...
        with contextlib.suppress(FileNotFoundError):
            ...
            cache_files = await loop.run_in_executor(None, glob.glob, os.path.join(self._recent_cache_dir, "*"))

            for cache_file in cache_files:
                timestamp = await aiofiles.os.path.getmfile(cache_file)
                if (
                    datetime.now()
                    - datetime.fromtimestamp(timestamp) > QUERY_LIMIT
                ):
    ...
    async def _is_recent_query(self, query: str) -> bool:
        ...
        return (
            await aiofiles.os.path.exists(cache_file)
            and (datetime.now() - datetime.fromtimestamp(await aiofiles.os.path.getmtime(cache_file)))
            < QUERY_LIMIT
        )

    async def _get_cache_for_query(self, query: str) -> dict | None:
        ...
        try:
            async with aiofiles.open(cache_file, encoding="utf8") as cache:
                return json.loads(await cache.read())
    ...
    async def _get(
        ...
        if is_valid_token or not refresh_token:
            ...
            if not ignore_recent_queries and await self._is_recent_query(url + params):
                _cache = await self._get_cache_for_query(url + params)
            ...
            if reply.status_code == 200:
                ...
                if reply.json():
                    ...
                    if not ignore_recent_queries:
                        await self._update_recent_queries(url + params, response)
                else:
                    # this mostly happens when datadis provides an empty response
                    _LOGGER.info("Got an empty response")
                    if not ignore_recent_queries:
                        await self._update_recent_queries(url + params)
    ...
    async def get_supplies(self, authorized_nif: str | None = None):
        ...
        if self._smart_fetch and not is_smart_fetch:
            ...
            while _start < end_date:
                ...
                consumptions = utils.extend_by_key(
                    consumptions,
                    await self.get_consumption_data(
                        cups,
                        distributor_code,
                        _start,
                        _end,
