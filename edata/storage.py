...
async def async_load_storage(cups: str, storage_dir: str | None = None):
    ...
    await aiofiles.os.makedirs(_subdir, exist_ok=True)

    async with aiofiles.open(_recent_cache, encoding="utf-8") as f:
        content = await f.read()
        return check_storage_integrity(utils.deserialize_dict(json.loads(content)))
...
async def async_dump_storage(cups: str, storage: defs.EdataData, storage_dir: str | None = None):
    ...
    res= json.dumps(utils.serialize_dict(check_storage_integrity(storage)))
    async with aiofiles.open(_recent_cache, "w", encoding="utf-8") as f:
        await f.write(res)
